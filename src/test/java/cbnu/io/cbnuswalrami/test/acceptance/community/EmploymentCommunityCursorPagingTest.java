...
@DisplayName("취업 정보 커뮤니티 최신순 페이징 조회 인수테스트")
public class EmploymentCommunityCursorPagingTest extends AcceptanceTestBase {

    @Autowired
    private CommunityFixture communityFixture;

    @DisplayName("조회 id인 next 값과 size 4개로 조회하면 status code 200과 4개의 values 를 리턴한다")
    @Test
    public void given_next_and_size_when__cursor_paging_then_status_code_200() {
        ...
        given(documentationSpec)
                ...
                .filter(document(
                        "employment-community-paging-get",
                        getRequestParametersSnippet(),
                        getResponseFieldsSnippet()
                ))
                .when()
                .get("/api/employment-community/cursor")
                .then()
